...
${CHE_VAR_ARRAY}"
    if [ "${CHE_MULTIUSER}" == "true" ]; then
      ...
      if [ "${SETUP_OCP_OAUTH}" == "true" ]; then
        OCP_IDENTITY_PROVIDER_ID=openshift-v3
        OCP_IDENTITY_PROVIDER_CLIENT_ID=kc-client
        OCP_IDENTITY_PROVIDER_CLIENT_SECRET=secret123
      ...
      if [ "${SETUP_OCP_OAUTH}" == "true" ]; then
        ...
        $OC_BINARY new-app -f ${BASE_DIR}/templates/multi/oauth-client.yaml -p REDIRECT_URI="http://${KEYCLOAK_ROUTE}/auth/realms/che/broker/${OCP_IDENTITY_PROVIDER_ID}/endpoint"
        ...
        ${OC_BINARY} exec ${KEYCLOAK_POD_NAME} -- /opt/jboss/keycloak/bin/kcadm.sh create identity-provider/instances -r che \
          ...
          -s config.clientId=${OCP_IDENTITY_PROVIDER_CLIENT_ID} \
          -s config.clientSecret=${OCP_IDENTITY_PROVIDER_CLIENT_SECRET} \
          -s config.baseUrl="${OPENSHIFT_ENDPOINT}" \
          -s config.defaultScope="user:full" \
          --no-config --server http://localhost:8080/auth --user $KEYCLOAK_ADMIN_USER --password $KEYCLOAK_ADMIN_PASSWORD --realm master
